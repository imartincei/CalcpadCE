...
# Neither repo tags the version its bundle declares, so _refs() falls through to
# HEAD and would follow an upstream relicence silently. Pinned to the newest
# commit in the shipped line; LICENSE there is byte-identical to what shipped.
REF_PINS = {
    ("npm", "jspreadsheet-ce"): "fe71a3f0c875fd38b8d1c84794ba7d83cab1b8f6",
    ("npm", "jsuites"): "8de9382f8d528f03de339c832548e93c9fe929aa",
}
...
def fetch_one(name, version, url, commit="", pin=""):
    """-> (ref, [(filename, url, text)]) for the first ref that yields anything."""
    m = REPO.match(url or "")
    if not m:
        return None, []
    host, owner, repo = m.groups()
    # A pin is exclusive -- falling back to HEAD would defeat it.
    refs = [pin] if pin else ([commit] if commit else []) + _refs(name, version)
    for ref in refs:
        hits = []
        for fn in NAMES:
...
def main(inventory, corpus_dir):
    ...
    for (eco, key), (name, version, url, commit) in sorted(targets.items()):
        pin = REF_PINS.get((eco, key), "")
        ref, hits = fetch_one(name, version, url, commit, pin)
        if not hits:
        ...
        pinned = ("pinned commit" if ref == pin else "build commit" if ref == commit
                  else "tag" if ref != "HEAD" else "default branch")
